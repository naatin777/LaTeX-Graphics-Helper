  ...
  const overlays = await Promise.all(
    canvasImages.map(async ({ bounds, image }, index) => {
      ...
      const sourceLeft = Math.max(
        0,
        Math.floor(((visibleLeft - bounds.x) / bounds.width) * imageMetadata.width),
      );
      const sourceTop = Math.max(
        0,
        Math.floor(((visibleTop - bounds.y) / bounds.height) * imageMetadata.height),
      );
      ...
      const sourceHeight = Math.min(
        ...
        Math.max(
          1,
          Math.ceil(((visibleBottom - visibleTop) / bounds.height) * imageMetadata.height),
        ),
      );
