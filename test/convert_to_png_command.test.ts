...
suite("PNGに変換コマンド", () => {
  ...
  test("JPEG、WebP、AVIF、SVG、PDF、Mermaidを1つのbatchでPNGへ変換する", async () => {
    ...
    try {
      ...
      const sourcePaths = [mmdPath, mermaidPath, ...imagePaths, svgPath, pdfPath];
      ...
      await Promise.all(
        [mmdPath, mermaidPath, ...imagePaths, svgPath].map((sourcePath) =>
