...
suite("Clipboard画像保存のartifact ownership", () => {
  test("rollback失敗時は復旧backupを保持し、Clipboard外をcleanupしない", async () => {
    ...
    try {
      ...
      await assert.rejects(
        ...
        (error: unknown) => {
          ...
          assert.match(
            ...
            /injected rollback copy failure/,
          );
          return true;
        },
      );
      ...
      assert.ok(
        lines.some((line) => line.includes("rollback failed") && line.includes(outputPath)),
      );
      assert.ok(
        lines.some(
          (line) => line.includes("preserving recovery backup") && line.includes(backupPath),
        ),
      );
