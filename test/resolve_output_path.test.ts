...
import { resolveOutputPath, type OutputPathContext } from "../src/config/resolve_output_path.js";
...
suite("出力パスのテンプレート解決", () => {
  ...
  test("Windowsのdrive letterとseparatorを許可する", () => {
    const result = resolveOutputPathWithPlatform(
      "${workspaceFolder}\\output\\result.pdf",
      windowsContext(),
      { platform: "win32" },
    );
  ...
  test("Windowsで多言語・絵文字・全角文字・全角空白を保持する", () => {
    ...
    const result = resolveOutputPathWithPlatform(
      `${"${workspaceFolder}"}\\output\\${fileName}`,
      windowsContext(),
      { platform: "win32" },
    );
  ...
  test("POSIXではWindows専用の禁止文字と予約名を許可する", () => {
    const result = resolveOutputPathWithPlatform(
      "${workspaceFolder}/output/CON?:*.pdf",
      posixContext(),
      { platform: "posix" },
    );
