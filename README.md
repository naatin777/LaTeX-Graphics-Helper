...
| Convert to Raw         | Raster images except `.raw`                                                                                  | `.raw` + `.raw.json`    | Export raw pixels with its sidecar metadata | None                                                             |
...
GIF/TIFF input uses only the first page/frame. Use the explicit animation preserve/split commands for multi-frame output. EPS and Raw output are supported by this release. Same-format conversion is rejected.
