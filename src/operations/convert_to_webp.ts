...
async function writeSourceAsWebp(
  ...
  await writeImageAsWebp(job.sourcePath, outputPath, job.workspacePath, signal);
}
...
async function writePdfPageAsWebp(
  ...
  await writeImageAsWebp(pngPath, outputPath, workspacePath, signal);
}
...
async function writeImageAsWebp(
  ...
  signal?.throwIfAborted();
  await sharp(sourceBuffer).webp().toFile(outputPath);
}
