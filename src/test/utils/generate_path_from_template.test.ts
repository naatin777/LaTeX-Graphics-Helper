...
suite('generate_path_from_template Test Suite', () => {
    ...
    setup(() => {
        clock = useFakeTimers(1755592839354);
    });
    ...
    test('should replace ${relativeFileDirname}', () => {
        ...
        const expected = path.join(
            path.dirname(path.relative(workspaceFolder.uri.fsPath, sourcePath)),
            'output.pdf',
        );
        assert.strictEqual(
            generatePathFromTemplate(template, sourcePath, workspaceFolder),
    ...
    test('should handle multiple replacements all', () => {
        ...
        const anotherWorkspaceFolder: vscode.WorkspaceFolder = {
        ...
        assert.strictEqual(
            generatePathFromTemplate(
                'abc/${fileBasename}' as TemplatePath,
                inputPath as Path,
                anotherWorkspaceFolder,
                page,
            ),
        ...
        assert.strictEqual(
            generatePathFromTemplate(
                '${fileBasenameNoExtension}.png' as TemplatePath,
                inputPath as Path,
                anotherWorkspaceFolder,
                page,
            ),
        ...
        assert.strictEqual(
            generatePathFromTemplate(
                'output${fileExtname}' as TemplatePath,
                inputPath as Path,
                anotherWorkspaceFolder,
                page,
            ),
        ...
        assert.strictEqual(
            generatePathFromTemplate(
                'output-${page}.pdf' as TemplatePath,
                inputPath as Path,
                anotherWorkspaceFolder,
                page,
            ),
        ...
        assert.strictEqual(
            generatePathFromTemplate(
                'output-${dateNow}.pdf' as TemplatePath,
                inputPath as Path,
                anotherWorkspaceFolder,
                page,
            ),
