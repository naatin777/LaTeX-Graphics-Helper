...
pre-package testはすべてVS Code Extension Hostで実行する。`pnpm run test`はbuild後に`vscode-test`を呼び、`out/test/**/*.test.js`を除外せずに1回実行する。plain Node + Mochaを直接起動するtest scriptは持たない。

`test.yml`はLinux、macOS、Windowsの各runnerで同じ`pnpm run test`を実行する。package済みVSIXのE2Eは引き続き別のElectron Playwright workflowで扱う。
...
- `pnpm run test`だけで全test scopeを実行できる
