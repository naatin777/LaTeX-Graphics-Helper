...
RuleSyncの生成、同期check、Stop hook、Current Task・Allowed files・Evidence matrixの機械的な必須検証は、v1の正式checkから外す。v1では`.rulesync/`、RuleSyncの設定、dependencyをrepositoryから削除し、RuleSyncを現在のrule正本やCIの成立条件として扱わない。
...
- RuleSyncの実行可能な設定やdependencyをrepositoryへ持ち込まない
