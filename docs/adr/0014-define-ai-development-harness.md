...
Stop hookは検証専用とする。dirty worktreeを理由に検証をskipせず、Current Taskのpreflight、RuleSync同期、軽量lint、format checkを実行する。hookから自動修正commandを呼ばず、結果はJSONで返し、詳細ログはGitの`lgh-harness` pathへ保存する。workspace runtime stagingへ保存してはならない。
