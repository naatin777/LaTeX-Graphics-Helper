...
- test数: 60件から33件
- Mocha: 14秒から11秒（約21%短縮）
- wall clock: 17.10秒から13.31秒（3.79秒、約22%短縮）

削除した27件は入力形式の検証を削ったものではない。6つの出力commandごとに主要入力を1つのbatchへまとめ、各入力の出力fileを個別に読み取って検証している。

### VS Code統合テスト全体

- test数: 165件から138件
- Mocha: 15秒から11秒（約27%短縮）
