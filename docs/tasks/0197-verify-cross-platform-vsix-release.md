...
- `package-vsix.mjs`のroot package、runner/target不一致拒否、`npx --no-install vsce package --target`、全platformでの`shell:false`を維持する。
...
- `check.yml`を`npm run check`だけにし、`check:test`の重複実行と存在しないscript参照を削除した。
...
- `npm run build` 成功
- `npm run test:webview` 成功
- `npm run package:vsix -- --out <macOS VSIX>` 成功
