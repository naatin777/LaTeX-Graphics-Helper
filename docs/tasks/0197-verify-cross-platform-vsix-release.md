...
- `test.yml`は3 OSでNode testとExtension Host testを各1回ずつ実行し、既存の外部画像toolのinstall・verificationを維持する。Playwright、VSIX package/install、releaseは実行しない。
- `playwright.yml`はPRと通常branch pushで、lockfile固定install、build、runner一致VSIXのpackage、隔離extensions directoryへのinstall、隔離user-dataでのVS Code起動、installed VSIXのElectron Playwright、全一時directory cleanupを3 OSで順に実行する。
...
- `test.yml`は3 OSのNode testとExtension Host testだけにし、外部画像toolのinstall・verificationは維持した。
...
- `pnpm run test:node` 成功（18件）
- `pnpm run test:vscode` 成功
