...
- `pnpm run ...`はscript実行前に依存状態を確認し、依存状態が古いと判断すると`pnpm install`を内部実行する。
...
pnpm 11.8.0では、project設定の書き込み先として`pnpm-workspace.yaml`が優先される。`pnpm config set --location project ...`で確認すると、`pnpm-workspace.yaml`にはcamelCaseのキーとして保存される。
