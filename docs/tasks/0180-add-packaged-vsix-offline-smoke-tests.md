...
- テスト終了時にVS Codeを閉じ、install先を含む隔離された一時directoryを削除する。VS Code CLIのuninstallはmetadataを残すOS差があるため、終了処理には使わない
- 既存の3 OS `vscode-test` matrixでplatform-specific VSIXを生成し、packaged smoke testをLinux、macOS、Windowsすべてで実行する。失敗時は各OSの`test-results/`をartifactへ保存する
