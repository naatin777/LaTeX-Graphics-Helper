...
Webviewの実操作とvisual testでは、実VS Codeが提供する`--vscode-*` CSS変数と`vscode-dark` / `vscode-light`などの`body` classを含む環境を正本にする。runnerを1つにすること自体を目的にせず、同じfixtureと検証helperを共有しながら、移行期間の役割分担を維持する。

### ブラウザPlaywright

- Webview単体のDOMと入力操作の高速な確認
- PDF.js canvasが描画されたこと
- zoom、scroll、crop box入力などの画面内操作
- WebviewからHostへ送るmessage
- Electron側へ移行するまでの既存coverage

ブラウザPlaywrightは、実VS CodeのCSS変数やtheme classを含むvisual testの正本にしない。Electron側の同等coverageが安定するまで直ちに削除せず、機能単位で移行後に削除する。
...
Playwright Electronは実VS Code windowとWebviewを一続きで操作できる。ただし、2026-07-10時点でPlaywright公式はElectron対応をexperimentalとしているため、段階導入する。既存dependencyの`@playwright/test`の`_electron`と`@vscode/test-electron`を使い、追加dependencyは導入しない。

最初はLinux CIでVS Code 1.128.0を固定し、LightはDefault Light Modern、DarkはDefault Dark Modernを使う。golden比較はLinuxの実Webview領域だけに限定し、失敗時はVS Code window全体のscreenshotをartifactへ残す。High ContrastとmacOS / Windowsのgoldenは対象外とする。

まず0153のharness smoke testで、起動・拡張読込・Crop Configureを開く・Webview到達・確実な終了だけを確認する。次に0154で、固定fixture `q a.pdf`を一時workspaceへコピーし、次のCrop Configureフローを検証する。

1. 全ページcanvasが描画されること
2. 実VS CodeのCSS変数が適用され、Dark / Lightのtheme classとcomputed styleが成立すること
3. crop値を入力してApplyすること
4. 出力PDFのpage count、MediaBox、CropBoxを確認すること

固定sleepは禁止する。DOM状態、theme class、computed style、file変更検知などの成立条件を待つ。Playwright ElectronがLinuxで安定化した後、macOS / Windowsへ広げる必要性を判断する。従来の「3 OSで安定した場合のみ採用する」という条件は採用しない。

`cropPdf.configure`のApply処理は`pdf-lib`であり、外部`pdfcrop` CLIではない。実`pdfcrop` CLIの確認はQuick cropの別タスクで扱う。Playwright screenshotはUI regression向けであり、出力PDFの内容検証を代替しない。基準画像はLinux、VS Code 1.128.0、指定themeに固定し、比較対象は実Webview領域だけにする。最終的には、Electron側の同等coverageが機能単位で安定してから、ブラウザrunner、Chromium install、関連workflowを廃止する。
