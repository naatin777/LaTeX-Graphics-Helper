...
| Image conversion | PDFに変換する出力形式基準コマンド                | `test/convert_to_pdf_command.test.ts`    | Covered      | PNG/JPEG/WebP/AVIF/SVG/Mermaid入力、複数入力、非対応入力、PDFサイズを検証 |
| Image conversion | editable Draw.io画像をPDFに変換する              | `test/png_safe_mode.test.ts`             | Covered      | `.drawio.png` / `.dio.png` / `.drawio.svg` / `.dio.svg`をrunner注入で検証 |
| Image conversion | MermaidをSVGに変換する出力形式基準コマンド       | `test/convert_to_svg_command.test.ts`    | Covered      | `.mmd` / `.mermaid`を検証                                                 |
