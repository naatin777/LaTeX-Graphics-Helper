...
| Image conversion | PNGをPDFに変換する                             | `test/convert_png_to_pdf.test.ts`        | Covered      | core変換を検証                                           |
| Image conversion | PNG/JPEG/WebP/AVIFをPDFに変換する              | `test/convert_to_pdf_command.test.ts`    | Covered      | command経由。1ページPDFとページサイズを検証              |
| Image conversion | SVGをPDFに変換する                             | `test/convert_to_pdf_command.test.ts`    | Covered      | command経由。1ページPDFとページサイズを検証              |
| Mermaid          | MermaidをSVGに変換する                         | `test/convert_to_svg_command.test.ts`    | Covered      | `.mmd` / `.mermaid`、SVG内容を検証                       |
| Mermaid          | MermaidをPDFに変換する                         | `test/convert_to_pdf_command.test.ts`    | Covered      | `.mmd` / `.mermaid`、PDFとして読めることを検証           |
| Command behavior | 変換メニューが出力形式基準で表示される         | `test/package_manifest.test.ts`          | Covered      | `変換 > PDF` / `変換 > SVG`のmanifest条件を検証          |
| Command behavior | 非対応入力を含むPDF変換を全体停止する          | `test/convert_to_pdf_command.test.ts`    | Covered      | 一部だけ変換しないことを検証                             |
...
| Config           | SVG→PDFの変換方式設定を読む                    | `test/convert_to_pdf_command.test.ts`    | Partial      | command経由のSVG変換で既定設定を検証。設定差し替えは薄い |
| Config           | Mermaid変換のChrome設定を読む                  | `test/convert_to_svg_command.test.ts`    | Partial      | 既定設定で実変換を検証。executablePath差し替えは未検証   |
