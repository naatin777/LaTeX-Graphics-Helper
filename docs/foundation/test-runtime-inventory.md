...
- 状態: 監査用draft
- 対象: 現在確認できたtest、runner config、package script
- 注意: GitHub connectorではdirectory treeを直接列挙できないため、PR差分・既知file・個別fetchから確認したinventoryである。未取得fileを「testなし」とは判定しない。
...
次は直接`vscode`をimportせず、contract上もVS Code runtimeを必要としない。
...
| `test/merge_pdf_operation.test.ts` | merge operation、Safe Mode、Undo、cancel | RT-VSCODE | RT-NODE | `rememberLastConversion`のcommand module依存は分離候補 |
...
- repository treeから全`*.test.ts`, `*.test.tsx`, `*.spec.ts`を列挙する
- Quick crop / Split /各output format commandの専用testを追加確認する
- Webview source内Vitest testの実在数を確認する
- test helperをruntime-specific / sharedへ分類する
- branch protectionでrequiredになっているworkflow gateを確認する
