...
- Current audit status: test inventory and document reconciliation are verified; required Evidence policy and platform scope remain maintainer Selection Gates.
...
| [Test file inventory](test-file-inventory.md)                       | local Git treeから全47 test file / 226 caseを列挙する                       |
| [Browser / Electron overlap](browser-electron-overlap.md)           | Browser 18 caseとElectron case groupのoracle差を比較する                    |
| [Evidence gaps](evidence-gaps.md)                                   | capability、spec、test、CI、platform、packagingの未接続を分類する           |
...
### O-005: Node候補はP0/P1/P2へ分ける必要がある

P0は、test contract、helper、source graphがNode-levelで、production boundary変更なしに実験できる候補である。代表例はsource format、Crop protocol、output path、file hash、workspace boundary、Safe Mode state、staged batch、commit、cleanupである。

P1はNode-level operationだが、native dependency、external process、platform simulation、またはfixture/harness調整の比較が必要である。external tool runner、PNG/native conversion、Draw.io route、ASCII scratch、auto crop、clipboard artifact operationが含まれる。

P2はNode移行候補ではなく、production boundary prerequisiteである。merge/Undo record owner、locale/environment、`SnippetString`、manifest constants、extension path、VS Code setting helper、generated Webview HTML/URI semanticsを含む。

詳細なfile/case単位の分類は[test-file-inventory](test-file-inventory.md)と[test-runtime-inventory](test-runtime-inventory.md)にあり、このaudit indexでは分類の意味とSelection Gateだけを保持する。重要度だけでNode候補とは判定しない。
...
- installed VSIX / controlled external-fetch failure
...
foundation auditで作成済みの[capability catalog](capability-catalog.md)と[spec / test trace](spec-test-trace.md)を、今回のinventoryで更新・検証した。
...
### O-012: test treeとcase inventoryはlocal repositoryから完了した

`origin/next/v1` at `75ca52a`を対象にlocal `git ls-files`、test構文、helper、fixture、snapshotを列挙した。

- root Mocha / Extension Host: 45 files、207 statically declared/configured cases
- Browser Playwright: 1 file、18 statically declared/configured cases
- Electron Playwright: 1 file、1 statically declared/configured case
- 合計: 47 files、226 statically declared cases

以前のfoundation文書にあった「未取得fileが残るためcompleteではない」という前提は訂正する。未解決なのはinventoryではなく、maintainerがrequired Evidenceの範囲を選ぶSelection Gateである。

### O-013: required runtimeはfile単位ではなくcase / dependency graph単位で仮説化する必要がある

直接`vscode`をimportしないtestでも、helperやsourceのtransitive dependencyによりHostで動いている例がある。一方、`vscode`をimportするtestでも、manifest static checkやextension path取得のようにcontractの一部だけがHostを必要とする例がある。詳細は[test-file-inventory](test-file-inventory.md)に記録した。

### O-014: `format:check`は現在のrepository scriptとして存在しない

要求された検証名`pnpm run format:check`は`package.json`に定義されていない。既存の`format`はsource / test / webview / config等を対象とするが、Markdown docsは対象外である。これは今回修正せず、CI / tooling task候補として記録する。
...
| Packaging smoke        | installed VSIX、native dependency、controlled external-fetch failure  | UI詳細、全変換組合せ                         |
...
- P0 strong Node candidatesを代表experimentする
- P1 Node experiment candidatesをnative / external process / platform別に比較する
...
| C-011 | 監査手順は`pnpm run format:check`を要求            | 現行package scriptに存在せず、既存formatもMarkdownを対象にしない |

## 9. Recommended sequence

### Phase 1: baseline completion

foundation auditで作成済み。今回のinventoryで更新・検証済み:
...
- branch protection / rulesetのrequired status確認（authenticated GitHub accessが必要）
...
- capability catalogを更新・検証した
- spec / test traceを更新・検証した
- representative testをruntime / oracleで分類した
- CI workflowをEvidence classで整理した
- tooling file coverageのdriftを記録した
- local Git treeから47 test file / 226 caseを完全列挙した
- Browser / Electronのcase単位比較を更新・検証した
- Node候補のhidden dependencyとHost残存根拠を再確認した
