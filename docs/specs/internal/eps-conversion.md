...
| 欠落       | 変換しない。エラーとして通知する            |
| 不正な値   | 非整数、範囲外、NaN → 変換しない            |
| `(atend)`  | Ghostscript依存。事前parseで検出しwarning   |

BoundingBoxの各値は次の範囲内とする。

- `llx`, `lly`, `urx`, `ury`: 整数（`−2^31` 〜 `2^31−1`）
- `llx < urx` かつ `lly < ury`
- `urx − llx` ≤ 出力上限inch × DPI
- `ury − lly` ≤ 出力上限inch × DPI
...
2. サイズが上限以下であることを確認する
3. 埋め込みfile操作（`file`、`renamefile`、`deletefile`）をPostScript codeから静的に検出しない。`-dSAFER` への信頼により対応する

完全なPostScript parseは行わない。構造確認はheader文字列確認とsize上限に留める。
...
変換入力の複雑さ（大量のvector data、埋め込みfont、複雑なtransparency）によっては、予測できない処理時間になる可能性があるため、timeoutは必須とする。

Timeout時は `AbortSignal` でGhostscript processを終了し、scratchを診断用に残す。

### Memory

Ghostscriptのmemory使用量を制限する。

- `-dMaxBitmap=10000000`（約10 MPixel）
- `-c "100000000 setvmthreshold"`（100 MBのVM threshold）

GhostscriptのVM exhaustion時はexit code 1で終了する。これを検出して「resource不足」として通知する。

### Disk

Ghostscriptの中間file出力を制限する。

- scratch rootの空き容量を事前確認しない。代わりに、変換後の生成PDF sizeが設定上限（初期値100 MB）を超えた場合はrejectする
- 生成PDFのsizeが上限を超えた場合、scratch内fileは診断用に残す
...
| 生成PDFが1 pageでない       | 変換不可                   | page数とsizeを記録      |
| 生成PDFのparse失敗          | 変換不可                   | error詳細を記録         |
| Timeout                     | 処理時間超過               | 実行commandを記録       |
| 生成PDF size超過            | 出力size超過               | 実sizeと上限を記録       |
