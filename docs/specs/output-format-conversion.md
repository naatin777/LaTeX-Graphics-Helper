...
初期実装では、`@mermaid-js/mermaid-cli`のNode.js APIではなく、dependencyとしてinstallされた`mmdc` CLIを呼び出す。
...
- CLI呼び出しの方が、外部コマンド実行として既存の変換処理に寄せやすい。
...
現時点では、出力形式基準コマンドのうち実装済みなのは`latex-graphics-helper.convertToPdf`だけである。そのため、各出力形式への対応は以下の実装に依存する。

- Mermaid → PDF: `convertToPdf`へMermaid入力を追加する
- Mermaid → SVG: `convertToSvg`を実装してMermaid入力を追加する
...
初期実装では、未実装の`convertToSvg`を追加し、Mermaid → SVGを先に実装する。
...
6. 残りのPDF/PNG/JPEG/WebP/AVIFへ段階的に拡張する
