...
Chrome executable pathは、SVG変換とMermaid CLIで共有する出力形式に依存しない`puppeteer.executablePath`設定を優先する。旧`convertToPdf.svg.puppeteer.executablePath`、`convertToPdf.mermaid.puppeteer.executablePath`、`convertToSvg.mermaid.puppeteer.executablePath`は、共通設定が未指定の場合だけfallbackとして使用する。browser channelは変換経路ごとの設定を維持する。
