...
対応形式は、editable Draw.io画像、PNG、JPEG、WebP、AVIF、GIF、TIFF、SVG、PDF、Mermaidの組み合わせとする。GIF（`.gif`）とTIFF（`.tif`、`.tiff`）は先頭page/frameだけを入力として扱い、animationやmulti-pageの展開は行わない。EPS（`.eps`）はsecurityとresource制限の設計が完了するまで対応しない。ネイティブDraw.io（`.drawio`、`.dio`）は、専用のPDF commandで扱う。対応していない入力、出力と同じ形式の入力、混在選択に含まれる非対応入力がある場合は、変換全体を開始しない。
...
GIF、TIFFは入力形式として追加し、GIF/TIFFの出力commandは追加しない。EPS（`.eps`）は入力形式として追加しない。EPSの変換経路とsecurity条件は[EPS変換の内部契約](../internal/eps-conversion.md)で定義する。
