...
| output template              | 出力の対象と粒度を示す                      | `outputPath.<format>`またはlegacy pair      | `outputPath.convertToPdf`、`outputPath.convertPdfToPng`               | `outputPath.result`                                                     | legacy pair-specific keyはADR-0020に従いv1系でfallbackとして残す            |
